...
public class Scenes {
	...
	static long f(int w, int ribbon) {
		// We're trying to create a mountain scene for which we don't have enough ribbon
		// material for.
		if (ribbon < 0) {
			return 0;
		}
		// When the width value exceeds the frame width, we know we’ve finished creating
		// a unique
		// mountain scene!
		if (w > W) {
			return 1;
